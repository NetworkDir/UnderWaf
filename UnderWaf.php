...
// UnderWaf v0.1
// Coded by @PacketLost
// Github : https://github.com/ElshanRajabi/UnderWaf
// -------------------------------------------------- //
// Functions
...
class UnderWaf {
  ...
  // Payloads
	function getArray($Type) {
		switch ($Type) {
			case 'SQL':
				return array("'",'´','SELECT FROM','SELECT * FROM','ONION','union','UNION','UDPATE users SET','WHERE username','DROP TABLE','0x50','mid((select','union(((((((','concat(0x','concat(','OR boolean','or HAVING',"OR '1",'0x3c62723e3c62723e3c62723e','0x3c696d67207372633d22','+#1q%0AuNiOn all#qa%0A#%0AsEleCt','unhex(hex(Concat(','Table_schema,0x3e,','0x00','0x08','0x09','0x0a','0x0d','0x1a','0x22','0x25','0x27','0x5c','0x5f');
				break;
			case 'XSS':
				return array('<img','img>','<image','document.cookie','onerror()','script>','<script','alert(','window.','String.fromCharCode(','javascript:','onmouseover="','<BODY onload','<style','svg onload');
				break;
			case 'LFI':
				return array('../', '../../', '../../../', '../../../../', '../../../../../', '../../../../../../');
				break;
			case 'RFI':
				...
				break;
			case 'RCE':
				return array('&cmd=', 'system(', 'whoami', '-la', 'mkdir', 'wget%20', 'exec(', '&lt;!--#exec%20cmd=&quot;/bin/cat%20/etc/passwd&quot;--&gt;', '<?php system("cat /etc/passwd");?>', 'system(%27', 'system(', '<?php', 'cat%20', 'cat%20/etc/passwd', 'get_user_file("/etc/passwd")', '$(`cat /etc/passwd`)', '%0a','%0a ping -i 30 127.0.0.1 %0a', 'ping%20', 'pwd','echo%20','|%20ls%20-l%20/', 'curl%20', '{{[] .__ Class __.__ base __.__ subclasses __ ()}}', 'cd%20');
				break;
			case 'CRLF':
				return array('%0d%0a');
				break;
			case 'BOT':
				return array('wget', 'sqlmap', 'nmap', 'arachni', 'util2pn', 'commix', 'nikto', 'sqlninja', 'sqliv', 'havij', 'whatweb', 'wpsscan', 'w3af', 'w3af.sf.net', 'w3af.org', 'python', 'cisco-torch', 'zeus', 'url_spider_pro', 'vega', 'vegabondo', 'vci', 'xget', 'xenu', 'wweb', '@nonymous', 'shodan', 'addresses.com', 'alexibot', 'grabber', 'netvampire', 'linkscan', 'magnet', 'superhttp', 'websucker', 'curl', 'python-requests', 'httrack');
				break;
	...
	function lfiCheck($Value, $Method, $DisplayName) {
		$Replace = array("can't" => "cant",
						"don't" => "dont");
		foreach ($Replace as $key => $value_rep) {
			$Value = str_replace($key, $value_rep, $Value);
		}
		$BadWords = $this->getArray('LFI');
		foreach ($BadWords as $BadWord) {
			// if (strpos(strtolower($Value), strtolower($BadWord)) !== false) {
			if(str_contains($Value, $BadWord)){
				$this->vulnDetectedHTML($Method, $BadWord, $Value, 'LFI (Local File Inclusion)');
			}
		}
	}
    function rfiCheck($Value, $Method, $DisplayName) {
		$Replace = array("can't" => "cant",
						"don't" => "dont");
		foreach ($Replace as $key => $value_rep) {
			$Value = str_replace($key, $value_rep, $Value);
		}
		$BadWords = $this->getArray('RFI');
		foreach ($BadWords as $BadWord) {
			// if (strpos(strtolower($Value), strtolower($BadWord)) !== false) {
			if(str_contains($Value, $BadWord)){
				$this->vulnDetectedHTML($Method, $BadWord, $Value, 'RFI (Remote file inclusion)');
			}
		}
	}
	function crlfCheck($Value, $Method, $DisplayName) {
		$Replace = array("can't" => "cant",
						"don't" => "dont");
		foreach ($Replace as $key => $value_rep) {
			$Value = str_replace($key, $value_rep, $Value);
		}
		$BadWords = $this->getArray('CRLF');
		foreach ($BadWords as $BadWord) {
			// if (strpos(strtolower($Value), strtolower($BadWord)) !== false) {
			if(str_contains($Value, $BadWord)){
				$this->vulnDetectedHTML($Method, $BadWord, $Value, 'CRLF (CRLF Injection)');
			}
		}
	}
	function rceCheck($Value, $Method, $DisplayName) {
		$Replace = array("can't" => "cant",
						"don't" => "dont");
		foreach ($Replace as $key => $value_rep) {
			$Value = str_replace($key, $value_rep, $Value);
		}
		$BadWords = $this->getArray('RCE');
		foreach ($BadWords as $BadWord) {
			// if (strpos(strtolower($Value), strtolower($BadWord)) !== false) {
			if(str_contains($Value, $BadWord)){
				$this->vulnDetectedHTML($Method, $BadWord, $Value, 'RCE (Remote code execution)');
			}
		}
	}
	...
	function botsCheck($Value, $Method, $DisplayName){
		$User_Agent = $_SERVER['HTTP_USER_AGENT'];
		$DenyList = $this->getArray('BOT');
		foreach($DenyList as $Parm){
		if(str_contains($User_Agent, $Parm)){
		$this->vulnDetectedHTML($Method, $Parm, $DisplayName, 'Bad Bots');
		}}
	}
		...
		foreach ($_GET as $key => $value) {
			if (is_array($value)) {
				...
				foreach ($flattened as $sub_key => $sub_value) {
					...
					$this->botsCheck($sub_value, "_GET", $sub_key);
					$this->rceCheck($sub_value, "_GET", $sub_key);
					$this->lfiCheck($sub_value, "_GET", $sub_key);
					$this->rfiCheck($sub_value, "_GET", $sub_key);
					$this->crlfCheck($sub_value, "_GET", $sub_key);
				}
			} else {
				$this->sqlCheck($value, "_GET", $key);
				$this->xssCheck($value, "_GET", $key);
				$this->htmlCheck($value, "_GET", $key);
				$this->botsCheck($value, "_GET", $key);
				$this->rceCheck($value, "_GET", $key);
				$this->lfiCheck($value, "_GET", $key);
				$this->rfiCheck($value, "_GET", $key);
				$this->crlfCheck($value, "_GET", $key);
			}
		}
	}
	function checkPOST() {
		foreach ($_POST as $key => $value) {
			if (is_array($value)) {
				...
				foreach ($flattened as $sub_key => $sub_value) {
					...
					$this->botsCheck($sub_value, "_POST", $sub_key);
					$this->rceCheck($sub_value, "_POST", $sub_key);
					$this->lfiCheck($sub_value, "_POST", $sub_key);
					$this->rfiCheck($sub_value, "_POST", $sub_key);
					$this->crlfCheck($sub_value, "_POST", $sub_key);
				}
			} else {
				$this->sqlCheck($value, "_POST", $key);
				$this->xssCheck($value, "_POST", $key);
				$this->htmlCheck($value, "_POST", $key);
				$this->botsCheck($value, "_POST", $key);
				$this->rceCheck($value, "_POST", $key);
				$this->lfiCheck($value, "_POST", $key);
				$this->rfiCheck($value, "_POST", $key);
				$this->crlfCheck($value, "_POST", $key);
			}
		}
	}
		...
		foreach ($_COOKIE as $key => $value) {
			if (is_array($value)) {
				...
				foreach ($flattened as $sub_key => $sub_value) {
					...
					$this->botsCheck($sub_value, "_COOKIE", $sub_key);
					$this->rceCheck($sub_value, "_COOKIE", $sub_key);
					$this->lfiCheck($sub_value, "_COOKIE", $sub_key);
					$this->rfiCheck($sub_value, "_COOKIE", $sub_key);
					$this->crlfCheck($sub_value, "_COOKIE", $sub_key);
				}
			} else {
				$this->sqlCheck($value, "_COOKIE", $key);
				$this->xssCheck($value, "_COOKIE", $key);
				$this->htmlCheck($value, "_COOKIE", $key);
				$this->botsCheck($value, "_COOKIE", $key);
				$this->rceCheck($value, "_COOKIE", $key);
				$this->lfiCheck($value, "_COOKIE", $key);
				$this->rfiCheck($value, "_COOKIE", $key);
				$this->crlfCheck($value, "_COOKIE", $key);
			}
		}
	}
	function gua() {
		...
		if ($this->CookieCheck == true) {
			...
			if (isset($_SESSION)) { // Session set.
				if (isset($_SESSION[$this->CookieCheckParam])) { // Logged.
					if (!(isset($_SESSION['underWaf-IP']))) {
						$_SESSION['underWaf-IP'] = $_SERVER[$this->IPHeader];
						return true;
					} else {
						if (!($_SESSION['underWaf-IP'] == $_SERVER[$this->IPHeader])) {
							// Changed IP.
							unset($_SESSION['underWaf-IP']);
							unset($_SESSION);
							@session_destroy();
							@session_start();
							return true;
